...
for index, summary_text in zip(list(input_df['Unnamed: 0']), list(input_df['Summary'])):
    ...
    rgp_importances = new_exps[:, -3:].mean(axis=0)
...
models = [
    LogisticRegression(max_iter=1000, random_state=0, solver=s, C=100, tol=0.0001) for s in names
]
...
with open('DATA/summary_stats.txt', 'a') as f:
    f.write(f'ROT global importance value is: {rot._explainer_model.g}\n')
